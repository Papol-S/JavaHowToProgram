package Lesson6.Exercise6_33;

import java.util.Random;
import java.util.Scanner;

public class CrapsGameModification {
    private static int bankBalance = 1000;

    public static int getBankBalance() {
        return bankBalance;
    }

    public static void setBankBalance(int bankBalance) {
        CrapsGameModification.bankBalance = bankBalance;
    }

    private static  final Random randomNumber = new Random();
    private enum Status {CONTINUE , WON , LOST}

    private static final int SNAKE_EYES = 2;
    private static final int TREY = 3;
    private static final int SEVEN = 7;
    private static final int YO_LEVEN = 11;
    private static final int BOX_CARS = 12;

    public static void main(String[] args) {
        Scanner input = new Scanner(System.in);
        System.out.print("Wagering ");
        int wager = input.nextInt();
        boolean checker = true;
        while ( checker) {
            if (wager > bankBalance) {
                System.out.print("Re enter Wagering ");
                wager = input.nextInt();
            }
            else {
                checker = false;
            }
        }

        int myPoint = 0;
        Status gameStatus;

        int sumOfDice = rollDice();

        switch (sumOfDice) {
            case SEVEN:
            case YO_LEVEN:
                gameStatus = Status.WON;
                break;
            case SNAKE_EYES:
            case TREY:
            case BOX_CARS:
                gameStatus = Status.LOST;
                break;
            default:
                gameStatus = Status.CONTINUE;
                myPoint = sumOfDice;
                System.out.printf("Point is %d\n",myPoint);
                break;
        }
        while ( gameStatus == Status.CONTINUE) {
            sumOfDice = rollDice();

            if (sumOfDice == myPoint)
                gameStatus = Status.WON;
            else if (sumOfDice == SEVEN)
                gameStatus = Status.LOST;
        }
        if (gameStatus == Status.WON) {
            setBankBalance(getBankBalance() + wager);
            System.out.println("Player wins");
            System.out.println(getBankBalance());
        }
        else {
            setBankBalance(getBankBalance()-wager);
            System.out.println("Player loses");
            System.out.println(getBankBalance());
        }
    }

    public static int rollDice () {
        int die1 = 1 + randomNumber.nextInt(6);
        int die2 = 1 + randomNumber.nextInt(6);

        int sum = die1 + die2;

        System.out.printf("Player rolled %d + %d = %d\n",die1,die2,sum);
        return sum;
    }


}
